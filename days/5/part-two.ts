...
function sortSteps(steps: string[], afterPages: PagesOrder) {
  // Bubble sort
  let isInOrder = true;
  ...
  for (let i = 0; i < steps.length - 1; i++) {
    for (let j = 0; j < steps.length - i - 1; j++) {
      ...
      if (!(currentAfterPages && currentAfterPages.includes(steps[j + 1]))) {
        const temp = steps[j];
        steps[j] = steps[j + 1];
        steps[j + 1] = temp;
        isInOrder = false;
    ...
    stepsLoop: for (let j = 0; j < steps.length; j++) {
      ...
      for (let k = 0; k < j; k++) {
        ...
        if (currentAfterPages.includes(steps[k])) {
          ...
          // Order the current steps
          sortSteps(steps, afterPages)
          break stepsLoop;
        }
      }
    }
